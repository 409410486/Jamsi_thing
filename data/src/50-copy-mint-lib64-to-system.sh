...
#	for file in gcc_s blkid ss selinux
	for file in gcc_s blkid uuid crypt selinux
    do
    	cp -a /lib/x86_64-linux-gnu/lib$file.so.* $HostLib/$LIB
	done

# libstdc++, libcrypto libform libpanel libmenu libncurses libxml2 libkmod
### /usr/lib
#	for file in jpeg png12 gssapi_krb5 k5crypto krb5support krb5 stdc++ edit \

	for file in stdc++ curl curl-gnutls ssl kmod
		do
...
# ssl cap attr
	...
	for file in com_err ncurses ext2fs z pcre
		do
			cp -a /lib/x86_64-linux-gnu/lib$file.so* $HostLib/$LIB
		done

#	cp -a /lib/x86_64-linux-gnu/libcrypt-* $HostLib/$LIB
